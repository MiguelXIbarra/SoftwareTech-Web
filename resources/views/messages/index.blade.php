            ...
            <div class="card-body py-3">
                <div class="row align-items-center">
                    ...
                    <div class="col-md-2 text-center">
                        ...
                        <span class="badge badge-light border px-2 py-1">{{ $message->created_at->format('d/m/Y H:i')
                            }}</span>
                    </div>
                    <div class="col-md-2 text-right">
                        <div class="btn-group shadow-sm">
                            <a href="{{ route('messages.show', $message->id) }}" class="btn btn-sm btn-white border"
                                title="Leer">
                                <i class="fas fa-envelope-open" style="color: #45a1b5;"></i>
                            </a>
                            @if($message->sender_id === Auth::id())
                            <a href="{{ route('messages.edit', $message->id) }}" class="btn btn-sm btn-white border"
                                title="Editar">
                                <i class="fas fa-edit" style="color: #ffc107;"></i>
                            </a>
                            <form action="{{ route('messages.destroy', $message->id) }}" method="POST"
                                id="delete-msg-{{ $message->id }}" style="display:inline">
                                @csrf @method('DELETE')
                                <button type="button" class="btn btn-sm btn-white border"
                                    onclick="confirmDelete('delete-msg-{{ $message->id }}')">
